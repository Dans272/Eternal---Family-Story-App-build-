  ...
  const handleProfileBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      store.setProfiles(prev => prev.map(p =>
        p.id === store.activeProfileId ? { ...p, bannerUrl: base64 } : p
      ));
      showToast('Banner updated');
    } catch (err) {
      console.error(err);
      showToast('Error processing banner');
    } finally {
      e.target.value = '';
    }
  };

  // Circle banner upload handler
  const handleCircleBannerUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const base64 = await compressImage(file);
      store.setCircleSettings(prev => ({ ...prev, bannerUrl: base64 }));
      ...
      case AppView.FAMILY_CIRCLE:
        ...
        return (
          <FamilyCircleView
            ...
            circleSettings={store.circleSettings}
            onBack={() => session.setView(AppView.HOME)}
            ...
            onBannerUploadClick={() => circleBannerInputRef.current?.click()}
            onBannerFileChange={handleCircleBannerUpload}
            bannerInputRef={circleBannerInputRef}
          />
        );
      ...
      case AppView.PROFILE:
        ...
        return (
          <ProfileView
            ...
            onBannerUploadClick={() => profileBannerInputRef.current?.click()}
            onBannerFileChange={handleProfileBannerUpload}
            bannerInputRef={profileBannerInputRef}
      ...
      case AppView.EDIT_PROFILE:
        return (
          <EditProfileView
            ...
            setEditImageUrl={editor.setEditImageUrl}
            editBannerUrl={editor.editBannerUrl}
            setEditBannerUrl={editor.setEditBannerUrl}
